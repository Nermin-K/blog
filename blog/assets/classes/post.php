...
class post
{
 ...
 private $connection;
    ...
    public function addPost($title ,$image ,$post ,$category, $userID)
    {
        ...
        $query = "INSERT INTO post (Title , Date_Time , Image , Post , Category , User_ID ) ".
            "VALUES ('{$t}' , '{$date}' , '{$i}' , '{$p}' ,'{$c}', {$user})";
      ...
      public function getOrderedPosts(){
        $query = " SELECT Title , Date_Time , Image , Post , User_ID , ID FROM post ORDER BY Date_time";
        $result = mysqli_query($this->connection , $query);
        if(!$result){ 
          return -1;
        }
   ...
   public function SearchPost($category,$title) {
	...
	if($category!="" && $title!="")
	{
		...
		$sql = "SELECT Post , ID FROM Post where Title='$title' and Category='$category'";
    ...
    public function UpdatePost($id,$updated_post,$title,$img,$cat) {
	
	$date = new DateTime();
	$sql = "UPDATE Post SET Post = '$updated_post' ,Title='$title', Date_Time= '{$date->format('Y-m-d H:i:s')}', Total_Rates=0, Raters_Number=0 , Category='$cat', Image='$img' WHERE ID='$id' ";
	$result = mysqli_query($this->connection , $sql);
